...
Read2::Read2(ros::NodeHandle& nodeHandle)
...
void Read2::initialize()
{
  if (!readFile(filePath_, pointCloudFrameId_)) ros::requestShutdown();

  if (isContinousPublishing_)
  {
    timer_ = nodeHandle_.createTimer(updateDuration_, &Read2::timerCallback, this);
  }
  else
  {
...
bool Read2::readFile(const std::string& filePath, const std::string& pointCloudFrameId)
{
...
void Read2::timerCallback(const ros::TimerEvent& timerEvent)
{
  if (!publish()) ROS_ERROR("Something went wrong when trying to read and publish the point cloud file.");
}
